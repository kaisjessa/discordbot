...
client.on('message', message => {
	...
	for(var i=0; i<keys.length; i++) {
	  ...
	  //-------------------Guests----------------------------------
	      ...
	      for(var i=0; i<keys.length; i++) {
		  var k = keys[i]
		  if(events[k].eventName.// TODO: oLowerCase() == message.content.substring(10).toLowerCase()) {
		     var userData = {};
		     userData[userName] = userId;
		     ref.child(k).child('guestlist').update(userData);
		  ...
		  k = keys[i];
		  var count = 1;
		  if(events[k].eventName.// TODO: oLowerCase() == message.content.substring(12).toLowerCase()) {
		     var attList = Object.keys(events[k].guestlist);
		     var idList = events[k].guestlist;
		     var bestList = [];
		     for(var j=0; j<attList.length;j++) {
	  ...
	  //--------------------------Help-------------------------------
		  ...
		  embed: {
			  ...
			  icon_url: client.user.avatarURL
				   ...
				   value: "Shows you // TODO: he guest list"
			       },
			       {
